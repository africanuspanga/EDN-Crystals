...
function SelectField({ label, error, options, className, ...props }: SelectProps) {
  return (
    <div className={cn("space-y-1.5", className)}>
      ...
      <select
        className={cn(
          "w-full rounded-lg border bg-white px-4 py-3 text-sm text-[#111827] focus:border-[#ce9f3d] focus:outline-none focus:ring-1 focus:ring-[#ce9f3d]",
          error ? "border-red-400" : "border-[#e5e7eb]"
        )}
        {...props}
      >
  ...
  if (status === "success") {
    return (
      <div className="rounded-2xl bg-[#edf2ff] p-8 text-center sm:p-12">
        <h3 className="text-xl font-semibold text-[#07132f]">Thank you</h3>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="rounded-lg border border-dashed border-[#e5e7eb] bg-[#f8fafc] p-4">
        ...
        <input
          type="file"
          disabled
          className="mt-2 block w-full text-sm text-[#667085] file:mr-4 file:rounded-full file:border-0 file:bg-[#2849a2] file:px-4 file:py-2 file:text-xs file:font-medium file:text-white disabled:cursor-not-allowed disabled:opacity-50"
        />
      ...
      <div className="space-y-1.5">
        <label className="flex items-start gap-3">
          <input
            type="checkbox"
            {...register("consent")}
            className="mt-1 h-4 w-4 rounded border-[#e5e7eb] text-[#2849a2] focus:ring-[#ce9f3d]"
          />
